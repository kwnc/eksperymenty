...
Dla implementacji AutoServe wykonano 3 próby generowania aplikacji, a następnie wyniki uśredniono. Takie podejście miało na celu upewnienie się, że nie występują odchylenia od wyników, które mogłyby zakrzywić obraz rzeczywistej wydajności systemu agentowego. Dzięki uśrednieniu trzech prób możliwe było zminimalizowanie wpływu losowych fluktuacji w generowaniu kodu przez modele językowe oraz uzyskanie bardziej wiarygodnych i powtarzalnych wyników.
...
W trakcie całego procesu wytwórczego system zużył łącznie 312k tokenów, co stanowi znaczący koszt obliczeniowy. Dla porównania, w eksperymentach z aplikacją notatnika system wieloagentowy przy zaawansowanej specyfikacji zużył 267k tokenów. Wyższe zużycie w przypadku AutoServe wynika z większej złożoności domeny biznesowej (zarządzanie serwisem samochodowym vs. notatnik), bardziej rozbudowanej specyfikacji oraz dłuższego procesu implementacji obejmującego pięć sprintów. Koszt ten należy uwzględnić przy ocenie opłacalności wykorzystania systemów wieloagentowych w komercyjnych projektach.
...
System wieloagentowy wykazał się szczególną skutecznością w wyborze odpowiedniego stosu technologicznego - decyzja o architekturze local-first była trafna dla przypadku użycia (brak potrzeby wdrażania oprogramowania na serwerze). Wybór Dexie.js jako warstwy abstrakcji nad IndexedDB oraz Zustand jako lekkiego state managera okazał się optymalny dla aplikacji MVP, pozwalając na bezproblemową implementację.

Analiza efektywności obliczeniowej wykazała, że generowanie aplikacji AutoServe wymagało 119 minut oraz 312k tokenów. W porównaniu z eksperymentami z aplikacją notatnika, gdzie system wieloagentowy przy zaawansowanej specyfikacji zużył 85k tokenów w 26 minut, AutoServe wymagał prawie czterokrotnie więcej tokenów i prawie pięciokrotnie więcej czasu. Różnica ta wynika z większej złożoności domeny biznesowej (zarządzanie serwisem samochodowym vs. notatnik), bardziej rozbudowanej specyfikacji obejmującej 63 funkcjonalności oraz dłuższego procesu implementacji podzielonego na pięć sprintów. Koszt ten należy uwzględnić przy ocenie opłacalności wykorzystania systemów wieloagentowych w komercyjnych projektach — mimo wysokiego zużycia zasobów, system dostarczył aplikację MVP o jakości produkcyjnej w czasie znacznie krótszym niż tradycyjne wytwarzanie oprogramowania.

W tabeli \ref{table:wyniki-wygenerowanej-aplikacji-biznesowej} przedstawiono wyniki wygenerowanej aplikacji analogicznie do poprzedniego eksperymentu.
...
\begin{table}[h!]
    ...
    \textbf{brak błędów} & 4 \\
    \hline
    \textbf{czas generowania} & 119 m \\
    \hline
    \textbf{liczba tokenów} & 312k \\
    \hline
    \end{tabular}

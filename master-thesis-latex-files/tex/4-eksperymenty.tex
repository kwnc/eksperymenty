...
Dla każdego scenariusza wykonano 3 próby generowania aplikacji, a następnie wyniki uśredniono. Takie podejście miało na celu upewnienie się, że nie występują odchylenia od wyników, które mogłyby zakrzywić obraz rzeczywistej wydajności systemu agentowego. Dzięki uśrednieniu trzech prób możliwe było zminimalizowanie wpływu losowych fluktuacji w generowaniu kodu przez modele językowe oraz uzyskanie bardziej wiarygodnych i powtarzalnych wyników.
...
Wyniki wskazują, że dla prostego celu (notatnik bez szczegółowej specyfikacji) najlepszy kompromis jakości i kosztu osiąga konfiguracja z jednym agentem: najwyższa ocena funkcjonalności i stabilności (brak błędów) przy akceptowalnej estetyce i prostym, czytelnym interfejsie. Układ architekt–programista poszerza zakres rozwiązania, lecz obniża jakość interfejsu i estetyki, mimo zachowania stabilności. Należy podkreślić, że żaden scenariusz nie wygenerował błędów, które przeszkodziłyby aplikacji w jej funkcjonowaniu.

Analiza czasu generowania oraz zużycia tokenów potwierdza przewagę pojedynczego agenta w prostych zadaniach. Scenariusz 1 ukończył pracę w 3 minuty przy zużyciu 6k tokenów, podczas gdy scenariusz 3 wymagał 24 minut i 95k tokenów — ośmiokrotnie dłużej i prawie szesnastokrotnie więcej zasobów obliczeniowych. Scenariusz 2 zajął pozycję pośrednią (9 minut, 31k tokenów). Wzrost kosztu obliczeniowego w konfiguracjach wieloagentowych wynika z komunikacji między agentami oraz wielokrotnego przetwarzania kontekstu, co przy prostych zadaniach nie przekłada się na proporcjonalny wzrost jakości.

W konsekwencji, dla zadań o niskiej złożoności zaleca się użycie pojedynczego agenta. Konfiguracje wieloagentowe mają uzasadnienie dopiero przy bogatszych wymaganiach wejściowych oraz jasno zdefiniowanych rolach.
...
\begin{table}[h!]
    ...
    \hline
    \textbf{estetyka} & 4 & 3 & 5 \\
    \hline
    \textbf{brak błędów} & 5 & 5 & 5 \\
    \hline
    \textbf{czas generowania} & 3 min & 9 min & 24 min \\
    \hline
    \textbf{liczba tokenów} & 6k & 31k & 95k \\
    \hline
    \end{tabular}
...
\subsubsection*{Scenariusz 1 - podstawowy agent}
Pojedynczy agent analizujący dokumentację był w stanie poprawnie wygenerować kod, który odpowiadał większości założeń specyfikacji. Czas generowania aplikacji wyniósł 15 minut.
...
Najlepsze wyniki uzyskano w autonomicznym układzie wieloagentowym — pełna funkcjonalność oraz dopracowany interfejs. Przy specyfikacji o takiej szczegółowości korzyści z podziału ról i współpracy agentów przeważają koszty koordynacji. Analiza efektywności czasowej pokazuje, że scenariusz 1 wykonał zadanie w 2 minuty przy 8k tokenów, scenariusz 2 potrzebował 10 minut i 41k tokenów, natomiast scenariusz 3 wymagał 25 minut oraz 84k tokenów. Mimo że system wieloagentowy zużył ponad dziesięciokrotnie więcej tokenów niż pojedynczy agent, przyrost jakości w kategoriach funkcjonalności i estetyki uzasadnia ten koszt przy zadaniach ze specyfikacją.

Wywnioskowano, że dla zadań opartych na podstawowej specyfikacji wieloagentowy system z jasnymi rolami jest najwydajniejszy. Gdy zasoby lub orkiestracja są ograniczone, kompromisem jest duet architekt–programista; pojedynczy agent pozostaje opcją do szybkich prototypów.
...
\begin{table}[h!]
    ...
    \textbf{brak błędów} & 5 & 5 & 5 \\
    \hline
    \textbf{czas generowania} & 2 min & 10 min & 25 min \\
    \hline
    \textbf{liczba tokenów} & 8k & 41k & 84k \\
    \hline
    \end{tabular}
...
Pod względem efektywności czasowej i zużycia zasobów, różnice między scenariuszami są wyraźne. Pojedynczy agent ukończył zadanie w 3 minuty przy 9k tokenów, scenariusz dwuagentowy potrzebował 12 minut i 45k tokenów, natomiast system wieloagentowy wymagał 26 minut oraz 85k tokenów. Przy złożonej specyfikacji wzrost kosztu obliczeniowego (prawie dziesięciokrotny w porównaniu z pojedynczym agentem) jest w pełni uzasadniony znaczącym przyrostem jakości — tylko scenariusz 3 osiągnął maksymalne oceny we wszystkich kluczowych kategoriach i uniknął błędów krytycznych.
...
\begin{table}[h!]
    ...
    \hline
    \textbf{czas generowania} & 3 min & 12 m & 26 m \\
    \hline
    \textbf{liczba tokenów} & 9k & 45k & 85k \\
    \hline
    \end{tabular}
...
Najbardziej złożona specyfikacja wymagała pełnego zespołu wyspecjalizowanych agentów. Tylko scenariusz wieloagentowy osiągnął pełnię funkcjonalności i zachował spójny interfejs, choć nadal wymagał dodatkowej kontroli jakości, aby ograniczyć błędy integracyjne.
...
Analiza kosztu obliczeniowego wykazała wyraźną zależność między liczbą agentów a zużyciem tokenów oraz czasem wykonania. W najprostszym eksperymencie pojedynczy agent zużył 6k tokenów w 3 minuty, podczas gdy system wieloagentowy wymagał 95k tokenów i 24 minut (prawie szesnastokrotny wzrost tokenów i ośmiokrotny wzrost czasu). W eksperymencie ze specyfikacją podstawową różnice były mniejsze: 8k tokenów w 2 minuty dla pojedynczego agenta wobec 84k tokenów w 25 minut dla systemu wieloagentowego (ponad dziesięciokrotny wzrost w obu metrykach). W eksperymencie ze złożoną specyfikacją: 9k tokenów w 3 minuty dla pojedynczego agenta wobec 85k tokenów w 26 minut dla systemu wieloagentowego (prawie dziesięciokrotny wzrost). Wzrost zużycia tokenów i czasu wynika z komunikacji między agentami, wielokrotnego przetwarzania kontekstu oraz mechanizmów koordynacji. Należy zatem rozważyć kompromis między jakością rozwiązania a kosztem obliczeniowym i czasowym, szczególnie w kontekście komercyjnego wykorzystania systemów agentowych.
...
\begin{figure}[!h]
    \centering \includegraphics[width=0.9\linewidth]{img/wnioski.png}
